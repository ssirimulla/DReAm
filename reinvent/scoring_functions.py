...
import threading
import pexpect
#rdBase.DiisableLog('rdApp.error')
########### ADDED ############################
...
############# OUR CLASS ################
class activity_model2():
    kwargs = ["clf_path"]
    clf_path = "data/rf.pickle"

    def __init__(self):
        voc_chars = ['0', 'r', '[c-]', '6', '4', '%', '[S-]', '\\', '[P@]', '1', '8', '[NH2+]',
                    '/', '-', '=', '[O-]', '[CH2-]', '[P+]', '[nH+]', '[C]', 'S', '[O]', '[S+]',
                    '.', '[cH-]', '#', '[C@@H]', '[S@]', 'o', '[P@@]', '[S@@]', '[Na+]', 'R',
                    'C', 'B', 'l', '[C-]', 'n', '[Li+]', '[c+]', '[C@@]', 'N', '[OH+]', '[SH]',
                    '[C@H]', '7', '[CH-]', '[S@@+]', '[O+]', '[NH-]', '[C+]', '(', '[N-]', '[NH+]',
                    '[s+]', '[N@H+]', '[n-]', '[nH]', '[H]', '[n+]', '[NH3+]', 'O', 'I', '9',
                    '[CH]', '[o+]', 'L', '[N]', 'P', 'F', '[N+]', 'c', 's', ')', '3', '2', '[S@+]', '[C@]', '5']

        self.chars = np.array(voc_chars, dtype=object)
        with open(self.clf_path, "rb") as f:
            self.clf = pickle.load(f)

    def __call__(self, smile):
        #smile = np.array(smile, dtype=object)
        print(smile)
        descriptors = self.get_one_hot_smile(smile)
        #print(descriptors, type(descriptors))
        return float(self.clf.predict(descriptors))

    #@classmethod
    def find_indices_bruteforce(self, smile, count):
        if len(smile) == 0 or count >= len(smile): return []
        
        # Bruteforce search
        frac = smile[:count]    
        if np.any(self.chars == frac):
            return [np.where(self.chars==frac)[0][0]] + self.find_indices_bruteforce(smile[count:], 1)
        else:
            return self.find_indices_bruteforce(smile, count+1)

    #@classmethod
    def find_indices(self, smile, count):
        if len(smile) == 0 or count >= len(smile): return []
    
        # Look for [*]
        squared_entries = []
        sm = "" # Strip string after finding squares
        pattern = "\[(.*?)\]"
        squares = [(m.start(0), m.end(0)) for m in re.finditer(pattern, smile)]
        if len(squares) > 0:
            frac = [smile[sq[0]:sq[1]] for sq in squares]
            for f in frac:
                if np.any(self.chars == f): 
                    squared_entries = squared_entries + [np.where(self.chars == f)[0][0]]
                    
            
            # Remove squares
            flag = 0
            for ind in squares:
                sm = sm + smile[flag:ind[0]]
                flag = ind[1]
        smile = sm
        
        return squared_entries + self.find_indices_bruteforce(smile, 1)

    #@classmethod
    def get_one_hot_smile(self, smiles):
        print(smiles)
        if type(smiles) == str:
            output = np.zeros((1, len(self.chars)))
            indices = self.find_indices(smiles, 1)
            print(indices)
            output[:, indices] = 1
            return output
        else:
            output = np.zeros((smiles.shape[0], len(self.chars)))
            for i, smile in enumerate(smiles):
                indices = self.find_indices(smile, 1)
                output[i, indices] = 1
            return output
...
def get_scoring_function(scoring_function, num_processes=None, **kwargs):
    ...
    scoring_function_classes = [no_sulphur, tanimoto, activity_model, activity_model2]
